...
public class RecommendationEngineTest {
    ...
    @Test
    public void testModerateSavingsRateRecommendations() {
        metrics.setSavingsRate(BigDecimal.valueOf(0.15));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.50));
        metrics.setNetCashFlow(BigDecimal.valueOf(200));
        
        List<String> recommendations = engine.generateRecommendations(metrics);
        
        assertFalse(recommendations.isEmpty());
        assertTrue(recommendations.stream().anyMatch(r -> 
            r.toLowerCase().contains("good start") || r.toLowerCase().contains("decent")));
    }
    
    @Test
    public void testPositiveCashFlowNoNegativeRecommendations() {
        metrics.setSavingsRate(BigDecimal.valueOf(0.15));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.70));
        metrics.setNetCashFlow(BigDecimal.valueOf(300));
    ...
    @Test
    public void testLowExpenseRatioNoHighExpenseRecommendations() {
        metrics.setSavingsRate(BigDecimal.valueOf(0.15));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.50));
        metrics.setNetCashFlow(BigDecimal.valueOf(200));
        ...
        assertFalse(recommendations.isEmpty());
        assertFalse(recommendations.stream().anyMatch(r -> 
            r.toLowerCase().contains("very high relative")));
    }
    
    @Test
    public void testZeroCashFlowBoundary() {
        metrics.setSavingsRate(BigDecimal.valueOf(0.15));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.85));
        metrics.setNetCashFlow(BigDecimal.ZERO);
        
        List<String> recommendations = engine.generateRecommendations(metrics);
        
        assertFalse(recommendations.isEmpty());
        // Should not have negative cash flow recommendations when exactly zero
        assertFalse(recommendations.stream().anyMatch(r -> 
            r.toLowerCase().contains("spending more than you earn")));
    }
    
    @Test
    public void testSavingsRateBoundaryConditions() {
        // Test exactly at 0.10 boundary (should use moderate recommendations)
        metrics.setSavingsRate(BigDecimal.valueOf(0.10));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.70));
        metrics.setNetCashFlow(BigDecimal.valueOf(100));
    ...
    @Test
    public void testExpenseRatioBoundaryConditions() {
        // Test exactly at 0.90 boundary
        metrics.setSavingsRate(BigDecimal.valueOf(0.10));
        metrics.setExpenseToIncomeRatio(BigDecimal.valueOf(0.90));
        metrics.setNetCashFlow(BigDecimal.valueOf(100));
